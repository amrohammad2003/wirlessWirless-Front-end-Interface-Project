...
function calculateEbNo(modulationType, ber) {
    ...
    let lowerBound = 0;
    let upperBound = 40;  // Increased upper bound for higher order modulations
    let tolerance = 1e-7;
    let eb_no_db;

    // Binary search in the dB domain for better precision across magnitudes
    while ((upperBound - lowerBound) > tolerance) {
        eb_no_db = (lowerBound + upperBound) / 2;
        let eb_no_linear = 10 ** (eb_no_db / 10);
        let calculatedBer = berFunction(eb_no_linear);
